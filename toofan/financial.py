...
class MySpider(scrapy.Spider):
	...
	def __init__(self, *args, **kwargs):
		...
		if not self.time_frame:
			print("No time frame Given to Crappy")
		if not self.company_name:
			print("No Company name Given to Crappy")
			
	def parse(self, response, *args, **kwargs):

		# referenceId : income, balance-sheet, cash-flow, ratio
		# reqType : S (for standalone), C (for consolidated)
		base_url = 'https://www.moneycontrol.com/mc/widget/mcfinancials/getFinancialData?classic=true&referenceId=income&requestType=S&scId='
		refId = self.refId
		reqType = self.reqType
#https://www.moneycontrol.com/mc/widget/mcfinancials/getFinancialData?classic=true&referenceId=income&requestType=C&scId=ACC&frequency=3
		company_name = self.company_name
		#company_name = 'SBI'
		#time_frame = 'frequency=3'
		time_frame = self.time_frame
		url = base_url + 'referenceId=' + refId + '&requestType=' + reqType + "&scId=" + company_name + '&' + 
...
process.crawl(MySpider,company_name='CI29',time_frame='frequency=3')
process.start()
